...
const reduceAttr = attributes => {
    ...
    const props = attrs.reduce((acc, { nodeName, nodeValue }) => {
...
const xmlToJson = ({ nodeType, childNodes, nodeName, nodeValue, attributes }) => {
    ...
    const children = Array.from(childNodes).map(node => xmlToJson(node));
